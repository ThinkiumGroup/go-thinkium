...
func (tab *Table) Lookup(targetID interface{}) []*Node {
	return tab.lookup(targetID.(common.NodeID), true)
}
...
func (tab *Table) lookup(targetID common.NodeID, refreshIfEmpty bool) []*Node {
	var (
		target         = common.Hash256(targetID[:])
...
func (tab *Table) findnode(n *Node, targetID common.NodeID, reply chan<- []*Node) {
	fails := tab.db.findFails(n.ID)
	r, err := tab.discv.FindNode(n.ID, n.UdpAddr(), targetID)
...
func (tab *Table) loadSeedNodes() {
	...
	for i := range seeds {
		...
		if config.IsLogOn(config.NetDebugLog) {
			log.Debugf("Found seed node in database id %s addr %s", seed.ID, seed.UdpAddr())
		}
		tab.add(seed)
	}
}
...
func (tab *Table) doRevalidate(done chan<- struct{}) {
	...
	if r := tab.replace(b, last); r != nil {
		if config.IsLogOn(config.NetDebugLog) {
			log.Debugf("Replaced dead node b %d id %s ip %s r %s rip %s", bi, last.ID, last.IP, r.ID, r.IP)
		}
	} else {
		if config.IsLogOn(config.NetDebugLog) {
			log.Debugf("Removed dead node b %d id %s ip %s", bi, last.ID, last.IP)
		}
	}
}
...
func (tab *Table) delete(node *Node) {
	...
	tab.deleteInBucket(tab.bucket(node.Hash), node)
}
...
func (tab *Table) addIP(b *bucket, ip net.IP) bool {
	...
	if !b.ips.Add(ip) {
		// if config.IsLogOn(config.NetDebugLog) {
		// 	log.Debugf("IP exceeds buckets limit, ip %s", ip)
		// }
		tab.ips.Remove(ip)
		return false
	}
